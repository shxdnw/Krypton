...
impl VaultService {
    ...
    pub fn create_vault(&self, password: &str) -> Result<()> {
        ...
        // Encrypt the verification blob *before* moving the cipher into
    ...
    pub fn unlock(&self, password: &str) -> Result<()> {
        // Step 1: read the salt from vault_meta *without* a cipher.
        ...
        // Step 2: derive the key and create the cipher.
        let cipher = cipher_from_password(
            self.deriver.as_ref(),
            password.as_bytes(),
            &salt,
        ...
        // Step 3: encrypt another verify blob with the same password to
        ...
        // Step 4: open the store and set the session.
        let store = Arc::new(SqliteStore::open(&self.store_path, cipher)?);
        store.init()?;
