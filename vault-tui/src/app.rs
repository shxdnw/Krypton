...
impl App {
    ...
    pub fn handle_action(&mut self, action: Action) {
        ...
        // If a confirmation dialog is active, route keys to it.
                ...
                if d.selected_yes {
                    match d.action {
                        ...
                        PendingAction::SaveEditedEntry => {
                            // Bypass the dialog on the actual save call.
                            ...
                            // Restore the dialog state if save failed (title empty).
                            if self.confirm_dialog.is_none() && matches!(self.state, AppState::Unlocked(View::EntryEdit(_))) {
                                // Save succeeded or failed with error — no need to re-show dialog.
                            }
                        }
                    }
                }
